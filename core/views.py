...
def process_text(request):  # pylint: disable=R0914 R1710 R0912 R0915
    ...
    if request.method == "POST":  # pylint: disable=R1710
        ...
        while see is True:
            ...
            cursor.execute(
                """INSERT INTO core_livro(titulo, data, highlight, anotacao)
                VALUES (%s, %s, %s, %s);""",
                [
                    dicionario["titulo"],
                    dicionario["data"],
        ...
        # cursor.execute("SELECT DISTINCT titulo FROM core_livro;")
...
@transaction.non_atomic_requests
def process_text_pt(request):  # pylint: disable=R0914 R1710 R0912 R0915
    ...
    if request.method == "POST":  # pylint: disable=R1710
        ...
        while see is True:
            ...
            if data[0:8] == "- Your N":
                ...
                counter += 1
            if not Livro.objects.filter(**dicionario).exists():  # pylint: disable=E1101
                cursor = connection.cursor()
                cursor.execute(
